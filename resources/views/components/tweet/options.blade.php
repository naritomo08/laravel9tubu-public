...
@if(Auth::check())
    ...
    @if(Auth::user()->is_admin)
        <details class="tweet-option relative text-gray-500">
            ...
            <div class="bg-white rounded shadow-md absolute right-0 w-24 z-20 pt-1 pb-1 dark:bg-gray-800">
                {{-- 管理者自身の投稿のみ編集 --}}
                @if($isAdminTweet)
                <div>
                    <a href="{{ route('tweet.update.index', array_merge(['tweetId' => $tweetId], $pageParameter)) }}" class="flex items-center pt-1 pb-1 pl-3 pr-3 hover:bg-gray-100 dark:hover:bg-gray-700">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
